...
{
	int smp=0; // текущий отсчёт
	int na,ac;
	int rb=0,rw=0;
	while(smp+con<tmgsize && smp+con <=n )
	{
		na=nonactual(tags,smp,con); // поиск неактуальных
		smp+=na;
		ac=actual(tags,smp,con); // поиск актуальных
		if(ac==con) break;
		smp+=ac;
	}
	if(smp+con>n)  die("no sequence");
	readwrite(smp,0);	// чтение неактуальных
	readwrite(con,1);	// чтение и запись актуальных
	readwrite(n-con-smp,0);	// чтение остатка
}
